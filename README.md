...
npm run balance   # 고정 시드 800회로 전략·경로별 난이도 측정
...
- 전투 보상 3장 중 1장 선택 또는 건너뛰기. 전투 보상에 자동 회복은 없습니다. 상단 보급품은 에너지 없이 체력 12 회복. 휴식은 체력 20, 훈련은 체력 6 회복입니다.
- 보스는 5턴부터 4턴마다 공격·흡수 수치가 6씩 늘어납니다. 전장의 폭주 예고와 턴 종료 예상 피해를 읽고 방어·약화·정화 시점을 고르세요.
- 지도 구역 이름을 눌러 앞으로 갈 경로를 미리 살펴볼 수 있습니다. 실제 이동은 현재 단계의 노드에서만 가능합니다.
...
난이도 조정 내역과 전략별 측정은 [BALANCE.md](BALANCE.md)에 있습니다. v0.3은 무료 에너지·과잉 회복을 줄이고 적과 보스전을 강화했습니다. 타이틀·지도·카드는 삽화와 읽기 쉬운 카드 면이 중심이 되도록 다시 구성했습니다.

## 저장

v0.3에서도 기존 도감·설정과 진행 상태를 유지합니다. 카드 효과와 적 패턴은 새 규칙을 사용하지만 진행 중인 적의 HP는 저장값을 유지하므로, 전체 난이도 조정은 새 탐험에서 확인하세요.
...
| `scripts/balance-simulation.ts` | 고정 시드·두 경로·두 전략으로 전체 탐험 측정 |
...
- 신규 WebP·UI 에셋은 약 2.19 MiB, 전체 배포의 예상 압축 전송량은 약 2.23 MiB입니다. 타이틀에서 처음 사용하는 이미지 2장과 HTML/JS/CSS는 약 0.36 MiB입니다. 나머지 이미지는 해당 화면에서 요청합니다.
- 10–15분은 설계 목표입니다. 자동 전략 비교와 브라우저 플레이로 난이도를 조정했지만 사람 대상 소요 시간·승률 조사는 아직 하지 않았습니다.
