...
1999년 TV 애니메이션 기반의 **비공식 팬메이드 웹게임 v0.10.0**입니다. 반복 현상, 사건, 육성 수치와 진화 조건은 이 게임의 창작 설정입니다. 원작 이미지·음원 추출물을 사용하지 않았습니다.
...
`dist/`만 정적 호스팅합니다. `base: '/file-island-echoes/'`이며 스크립트·CSS·이미지가 동일한 프로젝트 경로를 사용합니다. 폰트는 로컬 웹폰트, 음악과 효과음은 직접 제작한 로컬 압축 파일을 Web Audio로 재생하며 외부 주소가 없습니다. 게임 화면은 하나의 문서 내부 상태로 전환되며 `/battle` 같은 서버 경로를 만들지 않습니다. 프로젝트 URL 및 `/file-island-echoes/index.html` 직접 접근과 새로고침을 지원합니다. 새로고침 후 **이어서 탐험**으로 저장 상태를 불러옵니다.
...
저장 버전은 3, 키는 `file-island-echoes:v3`입니다. 8명마다 독립된 탐험 하나를 보관하며 도감·설정은 공유합니다. 타이틀의 **다른 탐험 선택**이나 캐릭터 화면에서 이어갑니다. 다른 캐릭터를 시작해도 기존 탐험은 남고, 같은 캐릭터 재시작에만 교체 확인이 나옵니다.

기존 v2/v1은 자동으로 해당 캐릭터 슬롯에 이관합니다. 성공한 v3 기록 전에 원본을 삭제하지 않습니다. 손상 슬롯만 제외하고 정상 슬롯은 보존합니다. 저장 실패는 변경 전 상태로 복구하며 다른 탭의 변경을 감지하면 최신 기록을 다시 읽도록 안내합니다. 전체 8슬롯 JSON 백업·복원과 자세한 정책은 [SAVE.md](SAVE.md), 검증은 [v0.10 기록](docs/v0.10-validation.md)에 있습니다.
...
| `src/storage.ts` | 저장 검증·v1/v2 이관·8슬롯 원자 저장 |
...
- 최신 버전은 146개 자동 테스트를 통과했습니다. [v0.10 검증](docs/v0.10-validation.md)을 참조하세요. 초기 버전에서는 8명 모두의 3챕터 승리 가능성, 1,600회 전략 비교를 수행했습니다. [VALIDATION.md](VALIDATION.md)에 브라우저 조작과 결과를 기록했습니다.
...
추가 요청에 따라 나리와 함께 있는 가트몬과 단독 전투 가트몬의 털 질감을 제거했습니다. 두 이미지 모두 매끈한 셀 셰이딩으로 교체했으며 자세·의상과 게임 규칙은 유지했습니다. v0.8은 구현하지 않았습니다.

## v0.8 · 전투 화면

카드는 한 번 누르면 바로 사용합니다. 적을 눌러 대상을 먼저 바꾸고 카드 앞면의 비용·효과를 확인하세요. 별도 확인 버튼은 없습니다. 남색 회로 뒷면에서 앞면으로 배분·뒤집기, 카드 상승, 적 행동별 연출을 추가했습니다. 전투 진입은 지역/전투 제목부터 보이며, 상단 메뉴는 한 줄로 유지됩니다. 움직임 줄이기를 켜면 이동·회전 없이 같은 전투 결과로 진행합니다.

게임 규칙과 저장 schema v2는 유지합니다. [구현·검증 기록](docs/v0.8-validation.md). 로컬 실행은 `npm install` → `npm run dev`, 프로덕션은 `npm run build` → `npm run preview`입니다.


## v0.8.1 · 전투 정보 정리

팝업은 제목/닫기 버튼과 스크롤 본문을 분리했습니다. 전투는 제목 → 능력·적 분석 → 턴 순서이며, 에너지 위 중복 체력은 제거했습니다. 파트너와 적 모두 방어도·효과는 이미지 위, 이름은 이미지와 체력 사이입니다. 선택한 적은 발밑 이중 링으로 표시합니다. [검증 결과](docs/v0.8.1-validation.md). 전투 규칙과 저장 형식은 그대로입니다.


## v0.8.2 · 보상과 전투 HUD

보상 카드는 좁은 화면에서 같은 폭의 한 줄을 유지하며 좌우 스크롤·휠·키보드로 선택할 수 있습니다. 선택한 적은 청록색 바닥 타원과 은은한 선택광으로 표시합니다. 첫 턴 배너를 제거하고 문장 능력을 짧은 진행 표시와 `사용` 버튼으로 정리했습니다. 고유 중첩은 파트너 방어 위에서 확인하며 모바일 상단바는 48px+safe area입니다. 기존 저장의 guide 값은 계속 허용하고 전투 규칙·저장 형식은 유지합니다. [검증 기록](docs/v0.8.2-validation.md).

## v0.9.0 · 모바일 PWA

설정에서 홈 화면 설치와 업데이트를 확인할 수 있습니다. 설치된 앱 본체 및 방문한 에셋으로 오프라인 재실행이 가능하며, 업데이트는 전투 이후 직접 적용합니다. 터치 효과음 7종만 미리 다운로드하고 음악은 지연 로드합니다. 실행·캐시·기기 제한은 [PWA.md](PWA.md), 검증 결과는 [v0.9 기록](docs/v0.9-validation.md)을 참고하세요.
