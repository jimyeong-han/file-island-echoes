...
1999년 TV 애니메이션 기반의 **비공식 팬메이드 웹게임 v0.6.2**입니다. 반복 현상, 사건, 육성 수치와 진화 조건은 이 게임의 창작 설정입니다. 원작 이미지·음원 추출물을 사용하지 않았습니다.
...
문장 심볼, 초상화·엔젤우몬, 게임 심볼, 3개 책갈피 도움말, 에너지 가로 표기 및 상단 소리 전환 배치를 개선했습니다. 검증 기록: [v0.6.1](docs/v0.6.1-validation.md).

## v0.6.2 · 2026-09-11

가트몬 단독 전투 이미지의 자세를 수정했습니다.

## v0.7 · 저장 파일과 초기화

설정 → **데이터 관리 · 백업과 복원**에서 JSON을 내려받습니다. 불러올 때에는 파일의 캐릭터·챕터·기록을 미리 확인하고 **불러와 현재 기록 교체**를 눌러야 반영됩니다. 전체 초기화는 별도 확인 화면에서 실행하며 이 게임의 현재/구버전 저장 키만 삭제합니다. 초기화 후 백업 JSON을 다시 불러올 수 있습니다.

UTF-8 JSON envelope는 `app`, `formatVersion: 1`, `appVersion`, `exportedAt`, `payload`로 구성됩니다. 내부 schema v2와 기존 v1 이관을 지원하며 최대 1MiB입니다. 미래 파일 형식과 잘못된 파일은 거부하고, 탐험만 손상된 경우 도감·설정 복구를 먼저 안내합니다. 파일은 서버로 전송하지 않습니다. 게임 규칙·저장 schema는 변경하지 않았습니다. [검증 결과](docs/v0.7-validation.md).
