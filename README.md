...
카드는 한 번 누르면 바로 사용합니다. 적을 눌러 대상을 먼저 바꾸고 카드 앞면의 비용·효과를 확인하세요. 별도 확인 버튼은 없습니다. 남색 회로 뒷면에서 앞면으로 배분·뒤집기, 카드 상승, 적 행동별 연출을 추가했습니다. 전투 진입은 지역/전투 제목부터 보이며, 모바일에서는 손패를 보는 동안에도 체력과 한 줄 메뉴가 유지됩니다. 움직임 줄이기를 켜면 이동·회전 없이 같은 전투 결과로 진행합니다.
