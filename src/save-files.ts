import { defaultSettings, emptyArchive, loadSave, SAVE_KEY, V2_KEY, LEGACY_KEY, validRun, parseSaveJson } from './storage';
...
export const freshSave=():Save=>({version:3,runs:{},activeCharacterId:null,archive:emptyArchive(),settings:defaultSettings()});
...
function inspectTree(value:unknown,depth=0,budget={nodes:0}){
 if(depth>16||++budget.nodes>100000)fail('저장 파일의 구조가 너무 복잡합니다.');
...
function checkPayload(p:unknown):asserts p is Dict {
 if(!object(p)||![1,2,3].includes(p.version as number)||(p.version===3?!object(p.runs):!Object.hasOwn(p,'run'))||!object(p.archive)||!object(p.settings))fail('저장 본문이 손상되었거나 지원하지 않는 저장 버전입니다.');
...
export function prepareImport(text:string,byteLength=new TextEncoder().encode(text).length):SavePreview {
 if(byteLength>MAX_SAVE_BYTES||new TextEncoder().encode(text).length>MAX_SAVE_BYTES)fail('저장 파일은 8MB 이하만 불러올 수 있습니다.');
 let data:unknown;try{data=parseSaveJson(text);}catch(error){if(error instanceof Error&&!(error instanceof SyntaxError))fail(error.message);fail('JSON 형식이 올바르지 않습니다.');}
 inspectTree(data);
 ...
 if(Object.hasOwn(data,'app')){
  ...
  if(![1,2].includes(data.formatVersion as number))fail('지원하지 않는 파일 형식 버전입니다.');
  ...
  if(![1,2,3].includes(data.version as number))fail('게임 저장 파일이 아닙니다.');
 ...
 for(const slot of Object.values(save.runs)){slot.run=cleanRun(slot.run);if(!validRun(slot.run))fail('탐험 저장을 변환하지 못했습니다.');}
...
export function exportSave(save:Save,now=new Date()){
 ...
 return {filename,text:JSON.stringify({app:APP,formatVersion:2,appVersion:APP_VERSION,exportedAt:now.toISOString(),payload:save},null,2)+'\n'};
}
...
export function resetSave(storage?:Pick<Storage,'getItem'|'setItem'|'removeItem'>):boolean{
 ...
 try{
  ...
  const io=storage;
  previous=[SAVE_KEY,V2_KEY,LEGACY_KEY].map(k=>[k,io.getItem(k)]);
  // Remove legacy first so a failed primary deletion cannot resurrect it.
  storage.removeItem(LEGACY_KEY);storage.removeItem(V2_KEY);storage.removeItem(SAVE_KEY);return true;
 }catch{
