...
import {prepareTouchAudio} from './touch-audio';
import {Pwa} from './pwa';
import {crestHUD, resourceChip} from './crest-ui';
...
import { chapterUnlocked, loadSave, updateArchive, activeRun, slotFor, recentSlot, saveRun, activateSlot, SaveSession, SAVE_KEY } from './storage';
...
prepareTouchAudio();
const loaded=loadSave();const save=loaded.save;const session=new SaveSession(save);
if(!session.commit(true))loaded.warning=session.error;
...
let helpChapter=0;
let modalOpener:string|undefined,renderedModal='';
let mapZone:number|null=null;
let selected:CharacterId='tai', order:number[]=[];let pendingChapter:ChapterId|null=null;
...
window.addEventListener('resize',()=>syncGround(root));
root.addEventListener('load',()=>syncGround(root),true);
...
const pwa=new Pwa(()=>{if(modal==='settings')render();});
void pwa.start();
const updateSafe=()=>!motion.busy&&activeRun(save)!?.screen!=='battle'&&activeRun(save)!?.screen!=='evolution';
function persist(previous:Run|null,progress=true) {if(progress)updateArchive(save,previous);const ok=session.commit();warning=session.error;if(!ok){notice='';render();}return ok;}
function act(fn:(r:Run)=>void) {const r=activeRun(save);if(!r)return;const before=structuredClone(r);const discovered=save.archive.forms.length+save.archive.enemies.length+save.archive.events.length;fn(r);if(!persist(before))return;render();const after=activeRun(save)!;gameAudio.transition(before,after,currentAction,discovered<save.archive.forms.length+save.archive.enemies.length+save.archive.events.length);if(before.screen==='battle'&&after.screen==='battle'&&after.battle!.hand.length>(before.battle?.hand.length||0))void dealHand(before.battle!.hand.length);}
...
function title(){const r=recentSlot(save)?.run;return `<main class="title-screen">
 ...
 <div class="title-audio">${btn(save.settings.muted?'소리 켜기':'소리 끄기','sound','audio-start',`aria-pressed="${!save.settings.muted}"`)}<small>이 모험을 위해 만든 음악과 효과음</small></div><div class="start-actions">${r?btn(`<span>${CHARACTERS[r.characterId].name} · ${r.screen==='result'?'결과 보기':'이어서 탐험'}</span><small>${slotSummary(r)}</small>`,'continue:'+r.characterId,'title-menu')+btn('다른 탐험 선택 · '+Object.keys(save.runs).length+'명','slots','quiet wide'):''}${btn('<span>새 탐험</span><small>아이와 파트너 선택 →</small>','new','title-menu')}${btn('<span>탐험 안내</span><small>카드 전투와 진화</small>','help','title-menu')}</div><p class="playtime">한 판은 약 10~15분이며, 진행 상황은 자동으로 저장됩니다.</p></div>
...
function partnerHealth(r:Run){return `<div class="partner-health ${r.hp<=r.maxHp*.25?'health-danger':''}" aria-label="${FORMS[r.form].name} 체력"><span><b>${r.hp}</b> / ${r.maxHp}${r.hp<=r.maxHp*.25?' · 위험':''}</span><meter min="0" max="${r.maxHp}" value="${r.hp}" low="${r.maxHp*.25}" optimum="${r.maxHp}" aria-label="${FORMS[r.form].name} 남은 체력"></meter></div>`;}
...
async function dealHand(start:number,token?:number){
 const r=activeRun(save)!;if(!r?.battle||r.screen!=='battle'||start>=r.battle.hand.length)return;
...
async function animateCard(index:number){
 ...
 if(!persist(before))return;const after=structuredClone(r),token=beginMotion();
...
async function animateTurn(){
 const r=activeRun(save)!;if(motion.busy||r?.screen!=='battle')return;
 const before=structuredClone(r),frames:TurnFrame[]=[];endTurn(r,frames);if(!persist(before))return;
...
function battle(r:Run){const b=r.battle!;const {incoming,recoil,damage}=turnThreat(r);const rage=bossRageBonus(b.turn);return `<main class="battle-content">${battleHeading(chapterOf(r).zones[zoneAt(r.row)]+' / '+kindName(nodeOf(r)!.type),nodeOf(r)!.name,b.turn)}<div class="battlefield ${nodeOf(r)?.type==='boss'?'boss-field':''} ${attack?'action-flash effect-'+cardEffect:''} ${enemyActing?'enemy-action':''}">${background(nodeOf(r)?.type==='boss'&&r.chapterId==='file'?3:sceneAt(r))}${nodeOf(r)?.type==='boss'?`<div class="boss-clock">${icon('burden')} ${rage?`폭주 · 공격 +${rage}`:'폭주 전'}<span>${BOSS_RAGE.turn}턴부터 ${BOSS_RAGE.interval}턴마다 공격 +${BOSS_RAGE.damage}</span></div>`:''}<div class="data-particles" aria-hidden="true"><i></i><i></i><i></i><i></i></div><div class="fighter player ${hpFeedback}"><div class="fighter-label">${resourceChip(r)}<span>${icon('guard')} 방어 ${b.block}</span></div><div class="status-tags">${playerStates(r)}${r.corruption?`<span>${icon('corruption')} 오염 ${r.corruption} · 턴 종료 체력 소모</span>`:''}${r.form==='skull'?'<span>반동 · 턴 종료 체력 -2</span>':''}</div>${sprite(r.form,'player-sprite')}<div class="fighter-base"></div><strong class="fighter-name">${FORMS[r.form].name}</strong>${partnerHealth(r)}</div><div class="enemy-group">${b.enemies.map((e,i)=>{const a=enemyMotion?.enemy===i&&enemyMotion.action?enemyMotion.action:intent(r,i);return `<button class="fighter enemy ${enemyMotion?.enemy===i?'acting-enemy intent-'+enemyMotion.action?.type:''} ${b.target===i?'targeted':''} ${attack&&cardEffect==='attack'&&(impactTarget===i||impactTarget===-1)?'hit-target':''} ${e.hp<=0?'defeated':''}" data-action="target:${i}" ${e.hp<=0?'disabled':''} ${targetAttributes(ENEMIES[e.id].name,b.target===i,e.hp<=0)}><div class="intent ${a.type}">${e.hp<=0?'정화 완료':`${icon(a.type)} ${a.label} <b>${a.value}</b>${a.hits&&a.hits>1?' · '+a.hits+'연타':''}${a.extra?' · '+effectName(a.extra)+' '+a.amount:''}`}</div>${enemyMotion?.enemy===i&&enemyMotion.action?`<span class="enemy-effect" aria-hidden="true">${icon(enemyMotion.action.type)}${Array.from({length:Math.min(enemyMotion.action.hits||1,8)},(_,h)=>`<i style="--pulse-delay:${h*65}ms"></i>`).join('')}</span>`:''}<div class="fighter-label"><span>${icon('guard')} 방어 ${e.block}</span></div><div class="status-tags enemy-badges">${enemyStates(e)}</div>${sprite(e.id)}<div class="fighter-base"><i class="target-field" aria-hidden="true"></i></div><strong class="fighter-name">${ENEMIES[e.id].name}</strong><div class="enemy-health"><span>${e.hp} / ${e.maxHp}</span><meter min="0" max="${e.maxHp}" value="${e.hp}" aria-label="${ENEMIES[e.id].name} 체력"></meter></div></button>`;}).join('')}</div></div>${crestHUD(r)}<p class="motion-status" role="status" aria-live="polite">${motionLabel}</p><div class="hand-toolbar"><div class="energy-orb">${b.energy}<small>에너지</small></div><div class="threat-readout ${damage>=r.hp?'lethal':''}"><small>이번 턴 예상 피해</small><strong>${damage}${damage>=r.hp?' · 쓰러질 위험':''}</strong><span>공격 ${incoming} − 방어 ${Math.min(incoming,b.block)}${recoil?` + 오염·반동 ${recoil}`:''}</span></div><div class="pile-counts">뽑기 <b>${b.draw.length}</b><span> / </span>버림 <b>${b.discard.length}</b></div>${btn('턴 종료 →','end','primary end-turn')}</div><div class="hand" aria-label="손패" aria-busy="${motion.busy}">${b.hand.map((id,i)=>{let html=card(id,`card:${i}`,r,cardStats(r,id).cost>b.energy);if(dealStart>=0&&i>=dealStart)html=html.replace('class="game-card','aria-hidden="true" inert style="--deal-delay:'+Math.min(i-dealStart,9)*75+'ms" class="game-card is-dealing');return html;}).join('')||'<p class="empty-hand">손패를 모두 사용했습니다. 턴을 종료하세요.</p>'}</div><details class="battle-log"><summary>전투 기록 · ${esc(logEntries(b.log).at(-1)?.text||'')}</summary><ol>${logEntries(b.log).map(l=>`<li value="${l.number}">${esc(l.text)}</li>`).join('')}</ol></details></main>`;}
function eventScreen(r:Run){const s=storyOf(r);return `<main class="story-screen"><div class="story-art">${background(sceneAt(r))}${storyPortrait(r)}<span class="eyebrow">${chapterOf(r).zones[zoneAt(r.row)]}</span></div><div class="story-copy"><div class="eyebrow">${icon('event')} 이야기 이벤트</div><h1>${s.title}</h1><p class="speaker">${s.speaker}</p><blockquote>${s.quote}</blockquote><p>${s.body}</p><div class="choices">${s.choices.map((c,i)=>btn(`<strong>${c.label}<span>→</span></strong>`,`choice:${i}`,'choice')).join('')}</div></div></main>`;}
...
function modalContent(){const r=activeRun(save)!;if(modal==='data')return dataPanel(filePreview,dataMessage,fileReading);if(modal==='data-reset')return resetPanel(dataMessage);if(modal==='ability'&&r)return abilityModal(r);if(modal==='keywords')return keywordModal();if(modal==='retire'&&r)return `<h2>안전한 곳으로 돌아갈까요?</h2><p>${CHARACTERS[r.characterId].retry}</p>${btn('탐험을 마치고 돌아가기','retire-confirm','danger wide')}`;if(modal==='help')return help();if(modal==='slots')return `<h2>이어할 탐험</h2><div class="slot-list">${Object.values(save.runs).sort((a,b)=>b.updatedAt-a.updatedAt).map(s=>slotButton(s.run)).join('')}</div>`;
 if(modal==='new'){const r=slotFor(save,selected)?.run;return `<h2>${CHARACTERS[selected].name}의 탐험을 새로 시작할까요?</h2><p>${r?slotSummary(r):''}</p><p>이 캐릭터의 슬롯만 교체됩니다. 다른 7명의 슬롯과 공용 도감·설정은 유지됩니다.</p>${btn('이 캐릭터로 새로 시작','confirm-new','danger wide')}${btn('취소','close','quiet wide')}`;}
...
function render(){
 const r=presented||activeRun(save)!;
 ...
 const closingModal=!!renderedModal&&!modal;
 if(modal&&!renderedModal)modalOpener=focusedAction;
 const oldDialogBody=root.querySelector('.dialog-body');
 const dialogScroll=modal===renderedModal?oldDialogBody?.scrollTop||0:0;
 renderedModal=modal;
 ...
 queueMicrotask(()=>syncGround(root));
 root.innerHTML=header()+(warning?`<div class="save-warning" role="status">${esc(warning)}${session.changed()?btn('최신 기록 불러오기','reload-save','secondary'):''}</div>`:'')+
  ...
  (modal?dialogFrame(modalContent()):'');
 if(modal){
 ...
 const nextFocus=Array.from(scope.querySelectorAll<HTMLButtonElement>('button:not(:disabled)')).find(b=>b.dataset.action===(closingModal?modalOpener:focusedAction));
 ...
 const dialogBody=root.querySelector('.dialog-body');if(dialogBody)dialogBody.scrollTop=dialogScroll;
 ...
 if(motion.busy)root.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(b=>b.disabled=true);
 updateAudioStatus();
}
function start(ch:ChapterId){if(!chapterUnlocked(save,selected,ch))return;saveRun(save,newRun(Date.now()>>>0,selected,ch));if(!persist(null))return;mapZone=null;view='game';modal='';pendingChapter=null;render();window.scrollTo(0,0);}
root.addEventListener('click',e=>{const el=(e.target as HTMLElement).closest<HTMLButtonElement>('[data-action]');if(!el)return;if(el.disabled)return;const [action,value]=el.dataset.action!.split(':');currentAction=el.dataset.action!;notice='';
 if(motion.busy)return;
 if(session.changed()&&!['save-export','reload-save','settings','data','close'].includes(action)){warning='다른 탭에서 기록이 바뀌었습니다. 최신 기록을 불러온 뒤 계속하세요.';render();return;}
 if(action==='reload-save'){location.reload();return;}
 if(action.startsWith('pwa-')){if(action==='pwa-install')void pwa.install();if(action==='pwa-check')void pwa.check();if(action==='pwa-repair')void pwa.repair();if(action==='pwa-apply')void pwa.apply(updateSafe(),()=>persist(activeRun(save),false));return;}
 ...
 if(action==='close'){cancelFileRead();modal='';render();return;}if(action==='home'){if(!persist(activeRun(save),false))return;view='title';modal='';render();return;}if(action==='new'){view='select';render();return;}
 if(action==='slots'){modal='slots';render();return;}
 if(action==='confirm-new'){if(pendingChapter)start(pendingChapter);return;}
 if(action==='continue'&&Object.hasOwn(CHARACTERS,value)){if(!activateSlot(save,value as CharacterId)||!persist(activeRun(save),false))return;mapZone=null;view='game';modal='';render();if(activeRun(save)?.screen==='battle'){void dealHand(0);focusBattle();}return;}
 if(['settings','archive','deck','help','credits'].includes(action)){modal=action;render();if(action==='settings')void pwa.refreshMusicStatus();return;}
 if(['sound','motion'].includes(action)){const k=action==='sound'?'muted':'reducedMotion';save.settings[k]=!save.settings[k];persist(activeRun(save),false);gameAudio.mixer.configure(save.settings);if(k==='muted'&&!save.settings.muted)gameAudio.mixer.playInput('ui-confirm');render();return;}
 ...
 if(action==='node'){mapZone=null;act(r=>enterNode(r,value));window.scrollTo(0,0);if(activeRun(save)!?.screen==='battle'){void dealHand(0);focusBattle();}return;}
...
function enemyStates(e:NonNullable<Run['battle']>['enemies'][number]){return (['weak','burn','root','shock','exposed','mark','power'] as const).filter(k=>e[k]>0).map(k=>`<span>${({weak:'약화',burn:'화상',root:'속박',shock:'감전',exposed:'노출',mark:'표식',power:'강화'})[k]} ${e[k]}</span>`).join('');}
function playerStates(r:Run){const s=r.battle!.ability;return (['weak','burn','tax','sealed','regen','thorns','crestBuff'] as const).filter(k=>s[k]>0).map(k=>`<span>${({weak:'약화',burn:'화상',tax:'추가 비용',sealed:'문장 봉인',regen:'재생',thorns:'반격',crestBuff:'공격 강화'})[k]} ${s[k]}</span>`).join('')+(s.lastStand?'<span>치명 피해 보호</span>':'');}
...
function extraAction(action:string,value:string){if(action==='selection'){view='select';modal='';render();return true;}if(action==='select'&&Object.hasOwn(CHARACTERS,value)){selected=value as CharacterId;view='detail';render();window.scrollTo(0,0);return true;}if(action==='chapters'){if(view==='game'&&activeRun(save)!)selected=activeRun(save)!.characterId;view='chapters';modal='';render();window.scrollTo(0,0);return true;}if(action==='begin'&&Object.hasOwn(CHAPTERS,value)){if(slotFor(save,selected)){pendingChapter=value as ChapterId;modal='new';render();}else start(value as ChapterId);return true;}if(action==='intro-done'){act(r=>{if(r.screen==='intro')r.screen='map';});return true;}if(['ability','keywords','retire'].includes(action)){modal=action;order=[];render();return true;}if(action==='retire-confirm'){modal='';act(retire);return true;}if(action==='crest'){const r=activeRun(save)!;if(r&&crestReady(r)){if(['matt','koushiro','tk','kari'].includes(r.characterId)){order=[];modal='ability';render();}else act(r=>crest(r));}return true;}if(action==='retain'||action==='crest-mode'){act(r=>{if(crest(r,action==='retain'?+value:value))modal='';});return true;}if(action==='order'){if(!order.includes(+value))order.push(+value);render();return true;}if(action==='order-reset'){order=[];render();return true;}if(action==='order-confirm'){act(r=>{if(crest(r,order))modal='';});return true;}return false;}
...
root.addEventListener('input',e=>{
 ...
 gameAudio.mixer.configure(save.settings);void gameAudio.mixer.unlock();if(!persist(activeRun(save),false)){gameAudio.mixer.configure(save.settings);render();}
});
...
function applyFileState(next:typeof save,message:string){
 motion.finish();cancelFileRead();gameAudio.resetScene();session.accept(next);warning='';view='title';modal='';mapZone=null;selected='tai';order=[];pendingForm=null;attack=false;enemyActing=false;dataMessage=message;notice=message;render();window.scrollTo(0,0);
}
function handleDataAction(action:string){
 ...
 if(action==='save-import-confirm'){
  ...
  const next=filePreview.save;applyFileState(next,Object.keys(next.runs).length>0?'저장 파일을 불러왔습니다. 이어하기에서 모험을 계속할 수 있습니다.':'저장 파일의 도감과 설정을 불러왔습니다. 새 탐험을 시작할 수 있습니다.');return true;
 }
...
root.addEventListener('change',async e=>{
 ...
 try{
  if(file.size>MAX_SAVE_BYTES)throw new Error('저장 파일은 8MB 이하만 불러올 수 있습니다.');
...
// Let a mouse wheel reach off-screen rewards without trapping vertical page scroll.
root.addEventListener('wheel',event=>{
 const rail=(event.target as HTMLElement).closest<HTMLElement>('.reward-cards');
 if(!rail||event.ctrlKey||event.shiftKey||Math.abs(event.deltaX)>=Math.abs(event.deltaY))return;
 const delta=event.deltaY*(event.deltaMode===1?20:event.deltaMode===2?rail.clientWidth:1),before=rail.scrollLeft;
 rail.scrollLeft=Math.max(0,Math.min(rail.scrollWidth-rail.clientWidth,before+delta));
 if(rail.scrollLeft!==before)event.preventDefault();
},{passive:false});

root.addEventListener('focusin',event=>{
 const card=(event.target as HTMLElement).closest<HTMLElement>('.reward-cards .game-card'),rail=card?.parentElement;
 if(!card||!rail)return;
 // Native focus scrolling can snap to the previous card on narrow viewports.
 requestAnimationFrame(()=>{
  if(!card.isConnected||document.activeElement!==card)return;
  const left=rail.scrollLeft+card.getBoundingClientRect().left-rail.getBoundingClientRect().left-8;
  rail.scrollTo({left:Math.max(0,Math.min(rail.scrollWidth-rail.clientWidth,left)),behavior:'instant'});
 });
});

function slotSummary(r:Run){return `${CHAPTERS[r.chapterId].name} · ${r.row+1}번째 장소 · ${FORMS[r.form].name} · ${({intro:'출발 준비',map:'탐험 중',battle:'전투 중',event:'이야기 선택',rest:'휴식',reward:'보상 선택',evolution:'진화',result:'결과 확인'})[r.screen]}`;}
function slotButton(r:Run){return btn(`<strong>${CHARACTERS[r.characterId].name} · ${CHARACTERS[r.characterId].partner}${save.activeCharacterId===r.characterId?' · 현재':''}</strong><small>${slotSummary(r)}</small><span>${r.screen==='result'?'결과 보기':'이어하기'} →</span>`,'continue:'+r.characterId,'slot-button wide');}
window.addEventListener('storage',e=>{if(e.key===SAVE_KEY||e.key===null){if(session.changed()){warning='다른 탭에서 기록이 바뀌었습니다. 최신 기록을 불러온 뒤 계속하세요.';if(!motion.busy)render();}}});
