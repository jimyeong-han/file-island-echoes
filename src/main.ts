...
void pwa.start();
const updateSafe=()=>!motion.busy&&save.run?.screen!=='battle'&&save.run?.screen!=='evolution';
...
root.addEventListener('click',e=>{const el=(e.target as HTMLElement).closest<HTMLButtonElement>('[data-action]');if(!el)return;if(el.disabled)return;const [action,value]=el.dataset.action!.split(':');currentAction=el.dataset.action!;notice='';
 ...
 if(action.startsWith('pwa-')){if(action==='pwa-install')void pwa.install();if(action==='pwa-check')void pwa.check();if(action==='pwa-repair')void pwa.repair();if(action==='pwa-apply')void pwa.apply(updateSafe(),()=>writeSave(save));return;}
 ...
 if(['sound','motion'].includes(action)){const k=action==='sound'?'muted':'reducedMotion';save.settings[k]=!save.settings[k];persist(save.run);gameAudio.mixer.configure(save.settings);if(k==='muted'&&!save.settings.muted)gameAudio.mixer.playInput('ui-confirm');render();return;}
