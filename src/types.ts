...
export interface Card {
 name:string; cost:number; kind:CardKind; text:string; owner?:CharacterId; keywords?:string[]; rarity?:'basic'|'uncommon'|'evolved'; stage?:number;
...
export interface CrestAbilityDefinition { name:string; text:string; condition:string; modes?:{id:string;name:string;text:string}[] }
...
export interface EnemyDef {name:string;hp:number;color:string;shape:string;description:string;pattern:Intent[];rank:'normal'|'elite'|'boss';chapters:ChapterId[];regions:string[];feature:string;resist:{burn:number;weak:number;root:number};purifiable:boolean;encounter:string;victory:string;art:string}
