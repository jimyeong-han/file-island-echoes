...
document.querySelector('#qa-show')!.addEventListener('click',()=>{
 ...
 else if(['battle','energy-12','angewomon-battle','boss'].includes(target)){r.row=target==='boss'?10:0;enterNode(r,chapterOf(r).map[r.row][0].id);if(target==='energy-12')r.battle!.energy=12;if(target==='angewomon-battle')r.form='angewomon';}
